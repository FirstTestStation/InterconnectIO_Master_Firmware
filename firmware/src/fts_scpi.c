 ...
 * @date    2024
 *
 * @brief   Software code used to interpret the SCPI command received from external
 *
 *
 ...
 *
 *
 * @copyright Copyright (c) 2021, Jan Breuer. Simple BSD license.
 ...
 *        The answer is also saved on global variable to be analyzed on module test.c
 *
...
size_t SCPI_write(scpi_t* context, const char* data, size_t len)
{
    ...
    while (!uart_is_writable(UART_ID))
    {
        ...
        tight_loop_contents(); // Use this if you want to prevent the CPU from idling in an infinite
                               // loop
    }
...
void SCPI_Beep()
{
    gpio_put(GPIO_BEEP, 1); // Turn ON beeper
 ...
 * It is initialized to `0` and used in the SCPI (Standard Commands for Programmable Instruments)
 * context.
 */
 ...
 *
 * @param context SCPI instance
 * @param ctrl    The identifier of the SCPI control register
...
/**
 * @brief   Set Bit on SCPI register Questionable and Operation
...
void RegBitHdwrErr(reg_info_index_t index, bool scbit)
{
    ...
    if (!scbit)
    { // if bits need to be set or clear
    ...
    if (!scbit && mreg[index].sreg > 0)
    { // if required to update the secondary register
        SCPI_RegSetBits(&scpi_context, mreg[index].sreg, 1 << mreg[index].sbit);
    }

    if (!scbit)
    {                                                         // if error bit has been set, send error message
...
scpi_result_t Relay_Chanlst(scpi_t* context, uint16_t* array)
{
    ...
    if (SCPI_Parameter(context, &channel_list_param, TRUE))
    {
        ...
        /* the next statement is valid usage and it gets only real number of dimensions for the
         * first item (index 0) */
        if (!SCPI_ExprChannelListEntry(context, &channel_list_param, 0, &is_range, NULL, NULL, 0, &dimensions))
        {
            ...
            { /* if valid, iterate over channel_list_param index while res == valid (do-while cause
                ...
                { /* still can have multiple dimensions */
                    ...
                    arr_idx++; /* increment array where we want to save our values to, not necessary
                                  otherwise */
...
static scpi_result_t Callback_Relay_scpi(scpi_t* context)
{
    ...
    { // if returned value is expected
        do
        { // loop on array until list of value is completed
...
static scpi_result_t Callback_Relay_all_scpi(scpi_t* context)
{
    ...
    while (SCPI_ParamNumber(context, scpi_special_all_numbers_def, &paramRelay, FALSE))
    {
        ...
        if (paramRelay.special)
        {
            ...
            // Power relay. offset of 500,600 or 700 is added to distinguish from bank relay
            // numbering
            ...
            // Open collector. offset of 500,600 or 700 is added to distinguish from bank relay
            // numbering
            ...
            // Open collector. offset of 700,500 or 600 is added to distinguish from bank relay
            // numbering
    ...
    { // in case of i = 0
        ...
        {
            return SCPI_RES_ERR;
        } // return error flag due to error occurred
    }

    i = 0; // reset loop counter
    if (tag == BSTATE || tag == SESTATE || tag == PWSTATE || tag == OCSTATE)
    { // if returned value is expected
        do
        {                                         // loop on array until list of value is completed
...
static scpi_result_t Callback_Digital_scpi(scpi_t* context)
{
    ...
    res = digital_execute(tag, numbers[0], numbers[1], value, answer); // execute command
...
static scpi_result_t Callback_eeprom_scpi(scpi_t* context)
{
    ...
    if (mode == 'w' || mode == 'r')
    {
        ...
        { // if no error found.
            ...
            { // if varname found
                status = cfg_eeprom_rw(mode, members[i].offset, members[i].size, svalue,
                                       strlen(svalue)); // write or read data on eeprom
                if (status == NOERR)
                {
                    strncpy(&ee.data[members[i].offset], svalue,
                            members[i].size); // save parameter on eeprom structure
                    if (mode == 'r')
                    {
                        SCPI_ResultCharacters(context, &ee.data[members[i].offset],
                                              members[i].size); // return value
                    }
                }
            }
        }
...
static scpi_result_t Callback_com_scpi(scpi_t* context)
{
    ...
    if (tag == CSWD || tag == CSRD)
    {
        ...
        } // Read parameter mandatory
        if (tag == CSRD)
        {
            res = SCPI_Parameter(context, &param1, false);
        } // Read parameter, not mandatory
        if (res)
        {
            size_t i, j, ctr;
            j = 0;
            ...
            for (i = 0; i < strlen(ustr); i++)
            {
                ...
                {
                    ctr = j;
                } // save pointer to detect end of string
            }
    ...
    case CSRD:
        ecode = scpi_uart_write_read_data(winfo, ustr,
                                          SCPI_INPUT_BUFFER_SIZE); // write data, expect answer
        if (ecode != NOCERR)
        {
    ...
    case OW_STR_NOT_IDENTICAL:
    {
        answer = STR_ONEWIRE;
        break;
    }
    case OW_READ_WRITE_FAIL:
    {
        answer = WR_ONEWIRE;
        break;
    }
    case OW_WRITE_FAIL:
    {
        answer = WRITE_ONEWIRE;
        break;
    }
    case OW_READ_FAIL:
    {
        answer = READ_ONEWIRES;
        break;
    }
    case OW_NO_VALIDID:
    {
    ...
    case UART_PROT_NUM_NOTVALID:
    {
        answer = UART_NUMBER_ERROR;
        break;
    }
    case UART_PROT_LETTER_NOVALID:
    {
        answer = UART_LETTER_ERROR;
        break;
    }
    case UART_RX_TIMEOUT_MS:
    {
        answer = UART_RX_ERROR;
        break;
    }
    case UART_LASTCHAR_TIMEOUT_MS:
    {
        answer = UART_LASTCHAR_ERROR;
        break;
    }
    case UART_BUFFER_FULL:
    {
        answer = UART_RXBUFFER_ERROR;
        break;
    }
    case UART_NOT_ENABLED:
    {
        answer = UART_ENABLE_ERROR;
        break;
    }
    }
...
/**
 * @brief Callback function to interpret the synchronized communication command received from the
 * SCPI port
 *
 * @param context SCPI instance
 */
...
static scpi_result_t Callback_sync_com_scpi(scpi_t* context)
{
    ...
    if (tag == SPWD || tag == SPRD || tag == ICWD || tag == ICRD)
    {
        ...
        { // extract first parameter
            ...
            if (param1.type == SCPI_TOKEN_ARBITRARY_BLOCK_PROGRAM_DATA)
            {
                ...
                {
                    printf("Error: Arbitrary block data length is odd, expect even number, Length: "
                           "%d.\n",
                           lenblk);
                    ecode = ARB_ODD_ERR;
                }
            ...
            { // if parameter is number
                ...
                size_t plen = 0;
                uint64_t val = lval;
                if (param1.type == SCPI_TOKEN_HEXNUM)
                {
                    plen = param1.len / 2;
                } // hex numbers
                if (param1.type == SCPI_TOKEN_OCTNUM)
                {
                    plen = param1.len / 2;
                } // octal numbers
                if (param1.type == SCPI_TOKEN_BINNUM)
                {
                    plen = param1.len / 8;
                } // binary numbers
    ...
    {
        tag = 0;
    } // if error found, do not execute instruction
    ...
    if (retv == true)
    {                                   //!< if single value need to be returned
    ...
    {
        if (!wordsize)
        { // if bytes size need to be returned
    ...
    case SPI_MODE_NUM_NOTVALID:
    {
        val = SPI_MODE_ERROR;
        break;
    }
    case SPI_CS_NUM_ERROR:
    {
        val = SPI_CS_ERROR;
        break;
    }
    case MALLOC_FAILURE:
    {
        val = MEMORY_ALLOCATION_ERROR;
        break;
    }
    case SPI_TIMEOUT:
    {
        val = SPI_TIMEOUT_ERROR;
        break;
    }
    case SPI_NOT_ENABLED:
    {
        val = SPI_ENABLE_ERROR;
        break;
    }
    case ARB_ODD_ERR:
    {
        val = ARB_WORD_FORMAT_ERROR;
        break;
    }
    case I2C_MALLOC_FAILURE:
    {
        val = MEMORY_ALLOCATION_ERROR;
        break;
    }
    case I2C_GENERIC_ERR:
    {
        val = I2C_GENERIC_ERROR;
        break;
    }
    case I2C_TIMEOUT_ERR:
    {
        val = I2C_TIMEOUT_ERROR;
        break;
    }
    case I2C_ADDRESS_NACK:
    {
        val = I2C_ADDRESS_NACK_ERROR;
        break;
    }
    case I2C_DATA_NACK:
    {
        val = I2C_DATA_NACK_ERROR;
        break;
    }
    case I2C_BUS_ERR:
    {
    ...
    case I2C_NOT_ENABLED:
    {
        val = I2C_ENABLE_ERROR;
        break;
    }
    default:
    {
...
scpi_command_t scpi_commands[] = {
    ...
    {
        .pattern = "*TST?",
        .callback = SCPI_CallbackTstQ,
    }, // selftest
    {
        .pattern = "*WAI",
        .callback = SCPI_CoreWai,
    }, // wait for all pending operation to complete
