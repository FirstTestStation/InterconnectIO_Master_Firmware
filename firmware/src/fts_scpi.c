  ...
  while (SCPI_ParamNumber(context, scpi_special_all_numbers_def, &paramRelay, FALSE))
  {
    ...
    if (paramRelay.special)
    {
      switch (paramRelay.content.tag)
      {
        ...
        // Power relay. offset of 500,600 or 700 is added to distinguish from bank relay numbering
        ...
        // Open collector. offset of 500,600 or 700 is added to distinguish from bank relay numbering
        case SCPI_OC1:                // Offset 500 = Pico Slave #1
          array[i] = 500 + GPIO_OC1;  //  Add gpio of OC1  on array list to open/close
          i++;
          break;

        // Open collector. offset of 700,500 or 600 is added to distinguish from bank relay numbering
...
void init_scpi()
{
  ...
  SCPI_Init(&scpi_context, scpi_commands, &scpi_interface, scpi_units_def, SCPI_IDN1, SCPI_IDN2, SCPI_IDN3, SCPI_IDN4, scpi_input_buffer,
