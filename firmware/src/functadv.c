...
/**
 * @brief  function to read value from the I2C devices INA219 (current/power monitor). Function
 * called by a SCPI command
 *
...
void calibrate_power(float actual, float expected)
{
    ...
    {
        fprintf(stdout,
                "INA219,calibration not performed, cal factor identical, actual value: %.2f, "
                "expected value: %.2f \n",
                actual, expected);
    }
}
...
uint8_t cfg_eeprom_rw(char mode, uint32_t eeaddr, uint8_t eedatalen, char* data, uint8_t datalen)
{
    ...
    { // if data written on eeprom, compare value.
        ...
        for (int i = 0; i < eedatalen; i++)
        {
            ...
            {
                fprintf(stdout,
                        "Error byte Write-read at address 0x%02X: write value 0x%02X: read value: "
                        "0x%02X\n",
...
/**
 * @brief Read the entire contents of the configuration eeprom and store data on eeprom config
 * structure EEprom structure is global (defined on master.h)
 *
...
bool Boot_check()
{
    ...
    ret += i2c_read_blocking(i2c0, PICO_PORT_ADDRESS, &rxdata, 1,
                             false); // check I2C com with Pico  Slave_1
    ret += i2c_read_blocking(i2c0, PICO_RELAY1_ADDRESS, &rxdata, 1,
                             false); // check I2C com with Pico  Slave_2
    ret += i2c_read_blocking(i2c0, PICO_RELAY2_ADDRESS, &rxdata, 1,
                             false);                                   // check I2C com with Pico  Slave_3
...
bool IOBoard_Selftest()
{
    ...
    { // if fail to communicate, check i2c comm
        ...
        datar = i2c_read_blocking(i2c0, PICO_PORT_ADDRESS, &dataw, 1,
                                  false); // check I2C com with Pico  Slave_1
        if (datar < 0)
        {
        ...
        datar = i2c_read_blocking(i2c0, PICO_RELAY1_ADDRESS, &dataw, 1,
                                  false); // check I2C com with Pico  Slave_1
        if (datar < 0)
        {
        ...
        datar = i2c_read_blocking(i2c0, PICO_RELAY2_ADDRESS, &dataw, 1,
                                  false); // check I2C com with Pico  Slave_1
        if (datar < 0)
        {
    ...
    // Selftest PWR (INA219)
    /*******************************************************************************/
    datar = i2c_read_blocking(i2c0, INA219_ADDRESS, &dataw, 1,
                              false); // check I2C com with Pico  Slave_1
    if (datar < 0)
    {
    ...
    {                                                 // if error found
        ...
        SCPI_RegSetBits(&scpi_context, SCPI_REG_OPERC,
                        1 << OPER_SELFTEST_FAIL); // ser register bits
