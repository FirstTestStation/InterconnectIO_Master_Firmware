...
static bool read_eeprom(uint8_t* device_id, uint8_t* data_buffer, int start_address, int length)
{
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        // fprintf(stdout, "R:%02X ", (unsigned char) data_buffer[i]);
    }
...
uint8_t onewire_write_info(const char* info, int start_address)
{
    ...
    // Write the FULL NB_INFO size to clear any old data with null padding

    for (i = 0; i < NB_INFO; i += 8)
    {
        // Clear buffer with null characters
        memset(pinfo, '\0', 8);

        // Copy actual data if available
        if (i < nbinfo)
        {
            int bytes_available = nbinfo - i;
            int bytes_to_copy = (bytes_available < 8) ? bytes_available : 8;
            memcpy(pinfo, info + i, bytes_to_copy);
        }
        // else: pinfo stays as all nulls (clears old data)

        epos = start_address + i;
        uint8_t valid = write_eeprom_8bytes(id, pinfo, epos, 8);
        if (valid != 0)
        {
            fprintf(stdout, "Error on writing EEprom, error # %d\n", valid);
            return OW_WRITE_FAIL;
        }

        fprintf(stdout, "Address %d: %s ", epos, pinfo);
        fprintf(stdout, "Hex: ");
        for (int j = 0; j < 8; j++)
        {
            fprintf(stdout, "%02X ", (unsigned char) pinfo[j]);
        }
        fprintf(stdout, "\n");
    }

    fprintf(stdout, "Successfully wrote %d bytes (%d data + %d null padding to clear old data)\n", NB_INFO, nbinfo, NB_INFO - nbinfo);
