...
struct user_com
{
    ...
    uint8_t lastchr;      //!< content last character of a string. Used to detect the last character of
                          //!< the receive string
};
...
/**
 * @brief  function to configure the parameters of the user uart based on the data set on the uart
 * structure
 *
 */
...
/**
 * @brief  function to disable the user uart and configure developed pins reserved by UART to normal
 * GPIO, set as input.
 *
 */
...
/**
 * @brief Function to update the timeout time register on the structure, Timeout is used to define
 * the maximum time to wait for a new character received by uart.
 *
...
/**
 * @brief Function to set or reset the uart handshake RTS-CTS. When disable the pins reserved the
 * handshake are set for GPIO
 *
...
/**
 * @brief Function to set the uart protocol according to parameter. The order of each parameter is
 * not important
 *
...
uint8_t scpi_uart_set_protocol(char* str)
{
    ...
    uint8_t pay, stp,
        data;       //!< variable who content the validated value of parity, stop and data bits
    ...
    //!< loop to pass over each character of the string to extract the parity, stop bits and data
    //!< bits.
...
/**
 * @brief function who return the uart protocol used in communication. The string format is Data,
 * parity, stop
 *
...
/**
 * @brief function to receive char from uart. The char need to be received before timeout expire.
 * End of string character is returned if timeout occur
 *
...
/**
 * @brief function to send data to uart without checking to receive data. The last character is
 * saved for later.
 *
...
{                // write data only
    ...
    // u_com.lastchr = dwt[tcr - 1];  // identify the last valid character of string, expect CR or
    // LF fprintf(stdout, "lastchar Tx only: 0x%x\n", dwt[tcr - 1]);
    return NOCERR;
}
...
{ // write data, expect answer
    ...
    if (tcr > 0)
    {
        ...
        //  u_com.lastchr = dwt[tcr - 1];  // identify the last valid character of string, expect CR
        //  or LF
